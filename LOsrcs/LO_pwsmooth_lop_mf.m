function [dn,ds] = LO_pwsmooth_lop_mf(adj,add,dip,w1,n1,n2,ns,order,eps,ndn,nds,type_mf,ifsmooth,dn,ds)
% LO_pwsmooth_lop: Plane-wave smoothing and median filtering (benchmarked with Madagascar, exact
% the same)
%
...
[dn,ds] = LO_adjnull( adj,add,ndn,nds,dn,ds);
...
if adj %adjoint operator deactivated
    ...
    [dn,utmp]=LO_pwspray_lop(1,1,n12,n12*ns2,dn,utmp,dip,ns,n1,n2,order,eps);
    
else %smoothing and median filtering
    
    [dn,utmp]=LO_pwspray_lop(0,0,n12,n12*ns2,dn,utmp,dip,ns,n1,n2,order,eps);
    
    u=reshape(utmp,n1,ns2,n2);
    
    for i2=1:n2
        if type_mf==0   %MF
        u(:,:,i2)= LO_mf(u(:,:,i2),ns2,1,2);
        else            %SVMF
        u(:,:,i2)= LO_svmf0(u(:,:,i2),ns2,1,2);    
        end
    end
