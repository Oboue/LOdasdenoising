function x = LO_ifdct_wrapping(C, is_real, M, N)
...
if finest == 1;
    ...
    [wl_1,wr_1] = LO_fdct_wrapping_window(coord_1);
    [wl_2,wr_2] = LO_fdct_wrapping_window(coord_2);
...
for j = scales,
    ...
    for quadrant = 1:nbquadrants
        ...
        wl_left = LO_fdct_wrapping_window(coord_corner);
        [wl_right,wr_right] = LO_fdct_wrapping_window(coord_right);
        switch is_real
         case 0
        ...
        for subl = 2:(nbangles_perquad-1);
            ...
            coord_right = 1/2 + floor(4*M_vert)/(wedge_endpoints(subl+1) - wedge_endpoints(subl)) * ...
                (wrapped_XX - mid_line_right)./(floor(4*M_vert)+1 - wrapped_YY);
            wl_left = LO_fdct_wrapping_window(coord_left);
            [wl_right,wr_right] = LO_fdct_wrapping_window(coord_right);
            switch is_real
             case 0
              wrapped_data = fftshift(fft2(ifftshift(C{j}{l})))/sqrt(prod(size(C{j}{l})));
        ...
        wl_left =LO_fdct_wrapping_window(coord_left);
        [wl_right,wr_right] = LO_fdct_wrapping_window(coord_corner);
        switch is_real
         case 0
          wrapped_data = fftshift(fft2(ifftshift(C{j}{l})))/sqrt(prod(size(C{j}{l})));
          wrapped_data = rot90(wrapped_data,(quadrant-1));
